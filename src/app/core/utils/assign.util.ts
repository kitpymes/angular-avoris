...
LINK: https://github.com/AlexGalays/immupdate#update-nested-property
...
but this can get tedious pretty fast, especially once we reach 3 levels of nesting or more. deepUpdate to the rescue:
...
Using update which doesn't specifically deal with Arrays, so we want to prepare the updated Array instance beforehand.
import { update } from 'immupdate'
...
Abort the whole thing (the returned value is the same as the passed one)
import { deepUpdate } from 'immupdate'
...
Specify a default value to be used if the current value is null/undefined
import { deepUpdate } from 'immupdate'
